...
SCAN_PLATFORMS = {"FACEBOOK_PAGE", "FACEBOOK_INSTAGRAM", "TIKTOK", "YOUTUBE"}

# Metrics platforms (confirmed actual API type names)
METRICS_PLATFORMS = {"FACEBOOK_PAGE", "FACEBOOK_INSTAGRAM", "TIKTOK"}

# Display name mapping
PLATFORM_DISPLAY = {
    "FACEBOOK_PAGE":      "FB",
    "FACEBOOK_INSTAGRAM": "IG",
    "TIKTOK":             "TT",
    "YOUTUBE":            "YT",
}

BASE_URL = "https://api.agorapulse.com"
METRICS_SHEET_URL = "https://docs.google.com/spreadsheets/d/1K7RoUhYuU3WfR3hDAzOGWamtS-jt6oE3-EI4E5qbuTg/edit"
GOOGLE_CREDS_FILE = os.path.expanduser("~/Desktop/google_creds.json")
...
def api_get(path, params=None, retries=2):
    ...
    # Trim requests older than 30 minutes
    now = time.time()
    _request_times = [t for t in _request_times if now - t < 1800]

    # If approaching limit (>450 in last 30 min), pause until window clears
    if len(_request_times) >= 450:
        oldest = _request_times[0]
        wait = 1800 - (now - oldest) + 5
        print(f"  [Rate limit pause: {wait:.0f}s to clear window]")
        time.sleep(wait)
        _request_times = []
...
# ─── METRICS AGGREGATION (T-90, T-60, T-30) ─────────────────────

def aggregate_window(posts, since_ts, until_ts):
    """Sum metrics for posts within a time window."""
    reach = views = clicks = comments = shares = 0
    for post in posts:
        pub = post.get("publishingDate", "")
        try:
        ...
        if not (since_ts <= ts <= until_ts):
            continue
        reach    += int(post.get("reachCount",    0) or 0)
        views    += int(post.get("viewsCount",    0) or 0)
        clicks   += int(post.get("clicksCount",   0) or 0)
        # TikTok uses commentCount/shareCount, others use commentsCount/sharesCount
        comments += int(post.get("commentsCount", 0) or post.get("commentCount", 0) or 0)
        shares   += int(post.get("sharesCount",   0) or post.get("shareCount",   0) or 0)
    return {"reach": reach, "views": views,
            "clicks": clicks, "comments": comments, "shares": shares}


def fetch_audience_days(org_id, ws_id, uid, since_ts, until_ts):
    """Fetch audience report once for full window. Returns list of daily records."""
    path = (
        f"/v1.0/report/organizations/{org_id}"
        f"/workspaces/{ws_id}"
        f"/profiles/{uid}"
    ...
    data = api_get(path, {"since": since_ts, "until": until_ts})
    if not data:
        return []
    return data.get("data", []) if isinstance(data, dict) else data


def get_followers_for_window(audience_days, since_ts, until_ts):
    """From pre-fetched audience days, get last known followers in a window."""
    from datetime import datetime, timezone
    window_days = []
    ...
    for day in reversed(window_days):
        f = int(day.get("followersCount", 0) or 0)
        if f > 0:
            return f
    return 0
...
def run_metrics():
    """
    Pull per-client, per-platform metrics for T-90, T-60, T-30 windows.
    Writes to Google Sheets with one row per client.
    """
    print("=" * 60)
    print("CLIENT METRICS PULL — T-90 / T-60 / T-30")
    print(f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"Platforms: {', '.join(sorted(METRICS_PLATFORMS))}")
    print("=" * 60)
    ...
    # results[client_name][platform_display][window_label][metric]
    results = {}
    request_count = 0
    ...
    for org_id, ws_id in ORG_WORKSPACES:
        ...
        profiles = [p for p in profiles if p.get("profileType", "") in METRICS_PLATFORMS]
        print(f"  {len(profiles)} profiles to scan")
        ...
        for i, profile in enumerate(profiles, 1):
            ...
            try:
                # Pull full 90-day window in one request
                data = get_content_report(org_id, ws_id, uid, t90, t0)
                request_count += 1
                posts = []
                if data:
                ...
                win_results = {}
                has_any_data = False
                for win_label, win_since, win_until in windows:
                    agg = aggregate_window(posts, win_since, win_until)
                    win_results[win_label] = agg
                ...
                if has_any_data:
                    if name not in results:
                        results[name] = {}
                    results[name][plat] = win_results
                    # Print summary
                    t30_data = win_results["T-30→Now"]
                    print(f"reach={t30_data['reach']:,} views={t30_data['views']:,} comments={t30_data['comments']:,}")
                else:
                    print("no data")
    ...
    print(f"\n{'=' * 60}")
    print(f"METRICS PULL COMPLETE — {len(results)} clients")
    print("=" * 60)

    # ── Build header row ─────────────────────────────────────────
    metrics_list = ["Followers", "Reach", "Views", "Clicks", "Comments", "Shares"]
    plats        = ["FB", "IG", "TT"]
    ...
    for win in win_labels:
        for plat in plats:
            for metric in metrics_list:
                header.append(f"{win} {plat} {metric}")

    # ── Build data rows ──────────────────────────────────────────
    def g(client_data, plat, win, metric):
        return client_data.get(plat, {}).get(win, {}).get(metric.lower(), "")
    ...
    for client_name, plat_data in sorted(results.items()):
        ...
        for win in win_labels:
            for plat in plats:
                for metric in metrics_list:
                    row.append(g(plat_data, plat, win, metric))
        rows_out.append(row)
    ...
    try:
        ...
        tab = datetime.today().strftime("%Y-%m-%d") + " Metrics"
        ...
        print(f"\n✓ {len(rows_out)} clients written to tab: '{tab}'")
        print(f"→ {METRICS_SHEET_URL}")

    except ImportError:
        print("\ngspread not installed — saving to CSV instead")
        ts = datetime.now().strftime("%Y%m%d_%H%M")
        fn = f"client_metrics_{ts}.csv"
        with open(fn, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(header)
...
def pull_asana_comments():
    print("\n" + "=" * 60)
    print("PULLING ASANA COMMENTS")
    print("=" * 60)

    pat = os.environ.get("ASANA_PAT", "")
    if not pat:
        print("  Skipped — set ASANA_PAT env var to include comments")
        return {}

    # Get all tasks
    tasks, path = [], f"/projects/{ASANA_PROJECT_GID}/tasks?opt_fields=name,gid&limit=100"
    while path:
        data = asana_get(path)
        if not data:
            break
    ...
    print(f"  {len(tasks)} Asana tasks found")

    comments_map = {}
    for i, task in enumerate(tasks):
        data = asana_get(
            f"/tasks/{task['gid']}/stories"
            f"?opt_fields=type,text,created_at,created_by.name"
        )
        if not data or not data.get("data"):
            time.sleep(0.2)
            continue
        comments = [s for s in data["data"] if s.get("type") == "comment"]
        if comments:
            comments_map[task["name"]] = " | ".join(
                f"[{c.get('created_at','')[:10]}] "
                f"{(c.get('created_by') or {}).get('name','?')}: "
                f"{c.get('text','').replace(chr(10),' ')}"
                for c in comments[-10:]
            )
        if (i + 1) % 50 == 0:
            print(f"  [{i+1}/{len(tasks)}] pulling...")
        time.sleep(0.2)

    print(f"  ✓ {len(comments_map)} clients with comments")
    return comments_map

def match_asana_comments(sheet_name, comments_map):
    if sheet_name in ASANA_OVERRIDES:
        return comments_map.get(ASANA_OVERRIDES[sheet_name], "")
    short = sheet_name.split(" //")[0].split("(")[0].strip().lower()
    for aname, comments in comments_map.items():
        if short == aname.lower().split(" //")[0].split("(")[0].strip():
            return comments
    words = [w for w in short.split() if len(w) >= 4]
    for aname, comments in comments_map.items():
        if sum(1 for w in words if w in aname.lower()) >= min(2, len(words)):
            return comments
    return ""
...
def run_full():
    ...
    for org_id, ws_id in ORG_WORKSPACES:
        ...
        for i, profile in enumerate(profiles, 1):
            ...
            try:
                data  = get_content_report(org_id, ws_id, uid, t90, t0)
                request_count += 1
                posts = data.get("data", []) if isinstance(data, dict) and data else []

                # Single audience call for full 90 days — split in Python
                audience_days = fetch_audience_days(org_id, ws_id, uid, t90, t0)
                request_count += 1
                ...
                for win_label, win_since, win_until in windows:
                    agg = aggregate_window(posts, win_since, win_until)
                    agg["followers"] = get_followers_for_window(audience_days, win_since, win_until)
                    win_results[win_label] = agg
                    if any(v > 0 for v in agg.values()):
                        has_any_data = True
                ...
                if has_any_data:
                    if name not in results:
                        results[name] = {}
                    results[name][plat] = win_results
                    t30d = win_results["T-30→Now"]
                    print(f"followers={t30d['followers']:,} reach={t30d['reach']:,} views={t30d['views']:,}")
                else:
                    print("no data")
    ...
    print(f"\n✓ {len(results)} clients with metrics")

    # ── Step 2: Asana comments ───────────────────────────────────
    print("\n── STEP 2: ASANA COMMENTS ──")
    comments_map = pull_asana_comments()
    ...
    for client_name, plat_data in sorted(results.items()):
        ...
        for win in win_labels:
            for plat in plats:
                for metric in metrics_list:
                    row.append(g(plat_data, plat, win, metric))
        comments = match_asana_comments(client_name, comments_map)
        if comments:
            matched += 1
        row.append(comments)
        rows_out.append(row)
    ...
    try:
        ...
        print(f"\n✓ {len(rows_out)} clients written to tab: '{tab}'")
        print(f"✓ {matched} clients matched to Asana comments")
        print(f"→ {METRICS_SHEET_URL}")
    ...
    if cmd == "--discover-orgs":
        ...
        if result:
            ...
            if viral:
                ...
                fetch_thumbnails_from_slack(viral)
                generate_3d_showcase(viral)
    elif cmd == "--help":
        print("""
